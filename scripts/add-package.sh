...
docker compose --env-file ${MAJOR_VERSION}/env run -v "/$PWD:/out" "$IMAGE" -e "write.csv(installed.packages()[, c('Package','Version')], row.names=FALSE, file=paste0('/out/', \"$MAJOR_VERSION\", '/packages.csv'))"
