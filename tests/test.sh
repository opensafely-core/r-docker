...
if [ "${MAJOR_VERSION}" = "v1" ]; then
  ...
  run_test "${MAJOR_VERSION}" /tests /tests/.tests.R
elif [ "${MAJOR_VERSION}" = "v2" ]; then
  # Test all R packages can be attached then detached
  run_test "${MAJOR_VERSION}" /tests /tests/tests/test-loading-packages.R
fi

# Check that a basic Rcpp call runs successfully
run_test "${MAJOR_VERSION}" /tests /tests/tests/test-rcpp.R

# Check number of packages
run_test "${MAJOR_VERSION}" /tests /tests/tests/test-number-packages.R

# Check capabilities of arrow package
run_test "${MAJOR_VERSION}" /tests /tests/tests/test-arrow.R

# Test loading the 14 base packages
run_test "${MAJOR_VERSION}" /tests /tests/tests/test-loading-base.R
