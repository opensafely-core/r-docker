...
build version *update="":
    ...
    docker compose --env-file {{ version }}/env run --rm -v "/$PWD:/out" r -q -e "print(Sys.getenv()); print(Sys.getenv('MAJOR_VERSION')); write.csv(installed.packages()[, c('Package','Version')], row.names=FALSE, file=paste0('/out/', Sys.getenv('MAJOR_VERSION'), '/packages.csv'))"

    # render the packages.md file
    docker compose --env-file {{ version }}/env run --rm -v "/$PWD:/out" r -q -e 'rmarkdown::render("/out/scripts/packages.Rmd", output_dir = paste0("out/", Sys.getenv("MAJOR_VERSION")))'
