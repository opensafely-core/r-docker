...
build version *update="":
    ...
    docker compose --env-file {{ version }}/env run --rm -v "/$PWD:/out" r -q -e "print(Sys.getenv('MAJOR_VERSION')); write.csv(installed.packages()[, c('Package','Version')], row.names=FALSE, file=paste0('/out/', Sys.getenv('MAJOR_VERSION'), '/packages.csv'))"
