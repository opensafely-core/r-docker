...
build version package="nopackage":
    ...
    docker compose --env-file {{ version }}/env run --rm -v "/$PWD:/out" rimage -e "write.csv(installed.packages()[, c('Package','Version')], row.names=FALSE, file=paste0('/out/', \"$MAJOR_VERSION\", '/packages.csv'))"
