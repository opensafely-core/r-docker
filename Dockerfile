...
RUN --mount=type=cache,target=/cache,id=/cache-2404 if [ "$UPDATE" = "true" ]; then R -e 'install.packages(c("renv", "pak"), repos = \"$REPOS\", destdir="/cache"); options(renv.config.pak.enabled = TRUE); pkgs <- read.csv("packages.csv"); renv::install(pkgs$Package, repos = \"$REPOS\", destdir="/cache")';
