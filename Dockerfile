...
RUN --mount=type=cache,target=/cache,id=/cache-2404 if [ "$UPDATE" = "true" ]; then R -e 'install.packages(c("renv", "pak"), repos = \"$REPOS\"); options(renv.config.pak.enabled = TRUE); renv::install(TODO:biglisthere, repos = \"$REPOS\", destdir="/cache")';
